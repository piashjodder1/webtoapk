...
<head>
    ...
    <title>Forgot Password - {{ \App\Models\Setting::get('seo_title', config('app.name', 'Website To App Builder')) }}</title>
        ...
        <main class="relative z-10 flex items-center justify-center px-4 py-20">
            <div class="w-full max-w-md">
                <div class="rounded-3xl border border-slate-200 bg-white/80 backdrop-blur-xl p-6 sm:p-10 shadow-xl shadow-primary-600/10">
                    <h1 class="text-3xl font-bubblegum text-gray-900 text-center">Forgot your password?</h1>
                    <p class="mt-2 text-sm text-gray-500 text-center">No problem. Just let us know your email address and we will email you a password reset link.</p>
                    ...
                    @if (session('status'))
                        <div class="mt-4 rounded-lg bg-green-50 border border-green-200 p-4 text-sm text-green-600">
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    <p class="mt-6 text-center text-sm text-gray-500">
                        <a href="{{ route('login') }}" class="font-bold text-primary-600 hover:text-primary-700 transition">Back to login</a>
                    </p>
                </div>
            </div>
        </main>
